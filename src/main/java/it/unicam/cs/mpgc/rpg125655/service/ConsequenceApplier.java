...
import it.unicam.cs.mpgc.rpg125655.model.item.ItemType;
...
public class ConsequenceApplier {
    ...
    public void apply(Choice choice, Character player) {
        ...
        } else if (cons.getHealthDelta() < 0) {
            player.getStats().takeDamage(-cons.getHealthDelta());
        }

        // Oggetti ricevuti
        for (String itemId : cons.getItemsGiven()) {
            player.addItem(new Item(itemId, itemId, "", ItemType.KEY));
        }
